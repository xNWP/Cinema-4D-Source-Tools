...
def isParameterPack(tp):
    return tp.default and tp.default.endswith('ParameterPack<>')

def templateParamToStrSubstPack(p, nodef, onlyNames, pack):
    if isParameterPack(p):
        return 'maxon::ParameterPack<' + p.name + '...>' if pack else p.name + '...' if onlyNames else 'typename... ' + p.name
...
class Generator(object):
                ...
                if method.static:
                    ...
                    if method.const:
                        ...
                        rfd << cls.name + '* o_ = (' + cls.name + '*) this->' + ('MakeWritable(' + cowReset + ').GetPointer();' if cowMode else 'GetPointer();') \
                            << 'if (!(S::Handler::KIND & (' + ('maxon::VALUEKIND::DEEP_CONSTNESS | ' if cowMode else '') + 'maxon::VALUEKIND::NEVER_NULLPTR)) && MAXON_UNLIKELY(!o_)) return ' + nullptrReturn.replace('@', 'MAKE_WRITABLE_FAILED' if cowMode else 'NULLPTR') + ';'
                    if method.kind == 'MAXON_FUNCTION':
                        oPointer = 'o_'
                        if method.parseAnnotation('incompleteReturnType', False):
    ...
    def processClass(self, cls):
        ...
        if cls.interface:
            ...
            self.unresolvedInit << templatePrefix + 'maxon::Bool ' + tclass + '::' + mtableCls + '::InitUnresolved(maxon::NonvirtualInterfaceReference::UnresolvedAssembler assembler, ' + mtableCls + '* tbl)' << '{'
        ...
        if cls.interface:
            ...
            if hasNonvirtual:
                ...
                self.unresolvedInit << '#ifdef PRIVATE_MAXON_ASSEMBLE_UNRESOLVED' << 'return assembler(&' + ('_staticInterface' if cls.interface == Class.VIRTUAL else '_interface') + ', (maxon::Char*) tbl' + superSize + ', _returnTypes, SIZEOF(' + ('Static' if cls.interface == Class.VIRTUAL else '') + 'MTable) / SIZEOF(void*));' \
...
def done(args, results, sources, msg, err):
    ...
    def writeRegisterFile(i, results):
        ...
        if setUsedFrameworks:
            registerB += '\nstatic maxon::Id g_usedFrameworks[] = {' + ', '.join(['maxon::Id("' + f + '")' for f in sorted(args.frameworks)]) + '};\n'
        ...
        for file, prologue in [('interface_registration.cpp', '#define MAXON_REGISTRATION_EXCLUDE_REFLECTION\n#define MAXON_REGISTRATION_EXCLUDE_REFLECTION_NONVIRTUAL\n'),
            ...
            if file == 'interface_registration.cpp' or args.frameworkRegistrationIncludeReflection:
                for f in fwr.split(';'):
                    f = f.strip()
                    if f and f != 'true':
                        fwId = getFrameworkId(args.directory, f)
