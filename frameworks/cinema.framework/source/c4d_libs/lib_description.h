...
enum
{
	...
	DESC_RESOURCEPATH		= 400006007,					///< @markPrivate
	DESC_RESOURCELINE		= 400006008,					///< @markPrivate
...
struct DescriptionCheckDragAndDrop
{
	...
	DescriptionCheckDragAndDrop() : id(0), element(nullptr) { result = true; }
};

/// Message struct for ::MSG_DESCRIPTION_DELETED.
struct DescriptionInExDeleted
{
	DescID			id;						///< ID of the drop field.
	GeListNode	*element;			///< deleted element.
	Int32				index;				///< Index in List before delete.

	DescriptionInExDeleted() : id(0), element(nullptr), index(0) {}
};

/// Message struct for ::MSG_DESCRIPTION_ADDED.
struct DescriptionInExAdded
{
	DescID			id;						///< ID of the drop field.
	GeListNode	*element;			///< added element.
	Int32				index;				///< Index in List after add.

	DescriptionInExAdded() : id(0), element(nullptr), index(0) {}
};

/// Message struct for ::MSG_DESCRIPTION_SELCHANGED.
struct DescriptionInExSelChanged
{
	DescID			id;						///< ID of the drop field.

	DescriptionInExSelChanged() : id(0) {}
};
...
/// @since R17.032
/// @see Take System @ref page_article_baseoverride_takesystem_additional_messages.
//----------------------------------------------------------------------------------------
struct DescriptionAllowOverride
{
	...
	BaseList2D*		_parent;						///< Assigned this value with parent node if the override must be assigned to it and not to the original node, useful in case of branched structures.
