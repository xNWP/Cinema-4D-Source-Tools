...
/// @addtogroup group_colorchooserlibrary ColorChooser
...
//----------------------------------------------------------------------------------------
/// Converts a RGB @formatParam{color} to string formatted based on the color range defined in c4dprefs: @ref WPREF_COLOR_RGBRANGE
...
//----------------------------------------------------------------------------------------
/// Converts a RGB float color component in range [0.0, 1.0] to 8bit (range [0, 255]).
...
/// @param[in] colorComponent			The 8bit color component to be converted. Must be in range [0, 255].
...
/// @param[out] red								The converted 8bit red color component.
/// @param[out] green							The converted 8bit green color component.
/// @param[out] blue							The converted 8bit blue color component.
//----------------------------------------------------------------------------------------
void ColorFloatTo8Bit(const Vector& floatColor, Int& red, Int& green, Int& blue);

//----------------------------------------------------------------------------------------
/// Converts a RGB 8bit color (range [0, 255]) to float color in range [0.0, 1.0].
/// @param[in] red								The 8bit color component to be converted. Must be in range [0, 255].
/// @param[in] green							The 8bit color component to be converted. Must be in range [0, 255].
/// @param[in] blue								The 8bit color component to be converted. Must be in range [0, 255].
...
/// @param[in] colorComponent			The color component to be converted. Must be in range [0.0, 1.0].
/// @return												The converted 16bit color component.
//----------------------------------------------------------------------------------------
...
/// @param[in] colorComponent			The 16bit color component to be converted. Must be in range [0, 65535].
/// @return												The converted color component in range [0.0, 1.0].
//----------------------------------------------------------------------------------------
...
/// @param[in] floatColor					The color to be converted. Must be in range [0.0, 1.0].
/// @param[out] red								The converted 16bit red color component.
/// @param[out] green							The converted 16bit green color component.
/// @param[out] blue							The converted 16bit blue color component.
//----------------------------------------------------------------------------------------
...
/// @param[in] red								The 8bit color component to be converted. Must be in range [0, 65535].
/// @param[in] green							The 8bit color component to be converted. Must be in range [0, 65535].
/// @param[in] blue								The 8bit color component to be converted. Must be in range [0, 65535].
/// @return												The converted float color.
//----------------------------------------------------------------------------------------
...
/// @param[in] ryb								Set to true to use RYB (red-yellow-blue) HSV space, set to false to use RGB (red-green-blue) HSV space. RYB generates visually more nicer palettes, but is less accurate because is smaller that RGB space. This means different RGB values could be converted to the same RYB value.
...
//----------------------------------------------------------------------------------------
/// Generates a Split Complementary Color Harmony palette from @formatParam{color}. Contains the input color and the two analogous colors to its complementary color The complementary color is calculated rotating 180&deg; the hue of the original color in HSV space.<#palette description#>
/// @param[in] color							The color to generate the palette. This color will be added to the resulting palette as well, and converted to RYB space if enabled.
/// @param[in] ryb								Set to true to use RYB (red-yellow-blue) HSV space, set to false to use RGB (red-green-blue) HSV space. RYB generates visually more nicer palettes, but is less accurate because is smaller that RGB space. This means different RGB values could be converted to the same RYB value.
/// @param[out] palette						List to add generated colors to. Data is preserved.
/// @return												True on success.
...
/// @param[in] color							The color to generate the palette. This color will be added to the resulting palette as well, and converted to RYB space if enabled.
/// @param[in] colorCount					Number of colors to generate. Because of input color is added as well, the resulting palette will have colorCount + 1 colors.
/// @param[in] ryb								Set to true to use RYB (red-yellow-blue) HSV space, set to false to use RGB (red-green-blue) HSV space. RYB generates visually more nicer palettes, but is less accurate because is smaller that RGB space. This means different RGB values could be converted to the same RYB value.
/// @param[out] palette						List to add generated colors to. Data is preserved.
/// @return												True on success.
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
/// Generates a palette composed by a defined amount of colors interpolated between @formatParam{color1} and @formatParam{color2} in HSV color space. All 3 HSV values will be interpolated. Example
...
/// @param[in] colorCount					Number of colors to generate. Because of the two input colors are added as well, the resulting palette will have colorCount + 2 colors.
/// @param[in] ryb								Set to true to use RYB (red-yellow-blue) HSV space, set to false to use RGB (red-green-blue) HSV space. RYB generates visually more nicer palettes, but is less accurate because is smaller that RGB space. This means different RGB values could be converted to the same RYB value.
/// @param[out] palette						List to add generated colors to. Data is preserved.
/// @return												True on success.
//----------------------------------------------------------------------------------------
...
public:
	...
	/// @return												The number of colors stored in this group.
	//----------------------------------------------------------------------------------------
	Int GetColorCount() const;

	//----------------------------------------------------------------------------------------
	/// Returns the color at the given index.
	/// @param[in] index							The index of the color. Must be 0 <= index < GetColorCount().
	...
	/// @return												True if the color could be accessed.
	//----------------------------------------------------------------------------------------
	...
	/// @param[in] index							The index of the color. Must be 0 <= index < GetColorCount().
	/// @param[out] selected					Optionally this gets the selection status of the color. Set to @formatConstant{nullptr} to ignore.
	/// @return												A pointer to the color value or nullptr. Can be edited without side effects.
	//----------------------------------------------------------------------------------------
	...
	/// @param[in] selectedOnly				Set to true to get only the selected colors, set to false to get all colors.
	/// @return												@formatConstant{True} if success.
	//----------------------------------------------------------------------------------------
	...
	/// @return												@formatConstant{True} if the color could be changed
	//----------------------------------------------------------------------------------------
	...
	/// @param[in] merge							Set to @formatConstant{True} to merge the colors with current group colors. Use @formatParam{insertAt} to define the insertion position. Set to false to replace the content of the group.
	...
	/// @param[in] index							The index of the color. Must be 0 <= index < GetColorCount().
	/// @param[in] removeCount				Number of colors to be removed (if @formatParam{removeCnt} is higher than what is available at position RemoveColor() will succeed, but remove only the number of available colors).
	/// @return												True if the color was successfully removed.
	//----------------------------------------------------------------------------------------
	...
	/// @return												True if duplicated colors found.
	//----------------------------------------------------------------------------------------
	Bool HasDuplicatedColors();

	//----------------------------------------------------------------------------------------
	/// Removes duplicated colors in this group.
	/// @return												True if duplicated colors found and removed, otherwise false.
	//----------------------------------------------------------------------------------------
	...
	/// @return												True if the color selection status was successfully set.
	//----------------------------------------------------------------------------------------
	...
	/// @param[in] index							The index of the color. Must be 0 <= index < GetColorCount().
	/// @return												True if the color is selected, false if is unselected or index is invalid.
	//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
/// This class handles the swatch data stored globally, in a BaseDocument or a preset.
/// Holds 2 kinds of groups:
/// - A global color group that is shared between all c4d documents and has special methods to handle it.
/// - Many Document-based color groups: these groups are stored in a c4d document.
//----------------------------------------------------------------------------------------
...
public:
	...
	/// @param[in] global							If true Global Swatch Group will be loaded.
	...
	//----------------------------------------------------------------------------------------
	/// Loads global colors. Global colors are global to all c4d documents, are used in the current working c4d session and preserved between c4d sessions if the option WPREF_COLOR_MODE_SWATCHES_REMEMBER_GLOBAL is set to true (The GUI option is in c4d prefs dialog / Units / Color Chooser / Default Set / Swatches Mode / Remember Global Swatches Last Layout). If that option is false, when c4d is restarted, the default colors will be loaded @see ColorSwatchData::LoadDefaultGlobalGroup() / ColorSwatchData::SaveDefaultGlobalGroup()
	/// @see ColorSwatchData::SaveGlobalGroup() / ColorSwatchData::LoadDefaultGlobalGroup() / ColorSwatchData::SaveDefaultGlobalGroup() / ColorSwatchData::GetGlobalGroup()
	/// @return												@formatConstant{True} if success.
	//----------------------------------------------------------------------------------------
	Bool LoadGlobalGroup();

	//----------------------------------------------------------------------------------------
	/// Saves to global colors and updates GUI. Global colors are global to all c4d documents, are used in the current working c4d session and preserved between c4d sessions if the option WPREF_COLOR_MODE_SWATCHES_REMEMBER_GLOBAL is set to true (The GUI option is in c4d prefs dialog / Units / Color Chooser / Default Set / Swatches Mode / Remember Global Swatches Last Layout). If that option is false, when c4d is restarted, the default colors will be loaded @see ColorSwatchData::LoadDefaultGlobalGroup() / ColorSwatchData::SaveDefaultGlobalGroup()
	/// @see ColorSwatchData::LoadGlobalGroup() / ColorSwatchData::LoadDefaultGlobalGroup() / ColorSwatchData::SaveDefaultGlobalGroup() / ColorSwatchData::GetGlobalGroup()
	/// @return												@formatConstant{True} if success.
	//----------------------------------------------------------------------------------------
	Bool SaveGlobalGroup();

	//----------------------------------------------------------------------------------------
	/// Loads default global colors. Default global colors are loaded when C4D starts and if the c4d preference option WPREF_COLOR_MODE_SWATCHES_REMEMBER_GLOBAL is false (The GUI option is in c4d prefs dialog / Units / Color Chooser / Default Set / Swatches Mode / Remember Global Swatches Last Layout). They are stored in the file "defaultswatches" in the c4d prefs folder. If that file is missing, a hardcoded default color set will be loaded instead.
	/// @see ColorSwatchData::LoadGlobalGroup() / ColorSwatchData::SaveGlobalGroup() / ColorSwatchData::SaveDefaultGlobalGroup() / ColorSwatchData::GetGlobalGroup()
	/// @param[in] ignoreFile					If @formatConstant{True}, the file "defaultswatches" will be ignored and the hardcoded colors will be loaded instead.
	/// @return												@formatConstant{True} if success.
	//----------------------------------------------------------------------------------------
	Bool LoadDefaultGlobalGroup(Bool ignoreFile = false);

	//----------------------------------------------------------------------------------------
	/// Saves to default global colors. Default global colors are loaded when C4D starts and if the c4d preference option WPREF_COLOR_MODE_SWATCHES_REMEMBER_GLOBAL is false (The GUI option is in c4d prefs dialog / Units / Color Chooser / Default Set / Swatches Mode / Remember Global Swatches Last Layout). They are stored in the file "defaultswatches" in the c4d prefs folder.
	/// @note This doesn't update GUI.
	/// @see ColorSwatchData::LoadGlobalGroup() / ColorSwatchData::SaveGlobalGroup() / ColorSwatchData::LoadDefaultGlobalGroup() / ColorSwatchData::GetGlobalGroup()
	/// @return												@formatConstant{True} if success.
	//----------------------------------------------------------------------------------------
	Bool SaveDefaultGlobalGroup();

	//----------------------------------------------------------------------------------------
	/// Gets the global colors group. @see ColorSwatchData::LoadGlobalGroup() / ColorSwatchData::SaveGlobalGroup() / ColorSwatchData::LoadDefaultGlobalGroup() / ColorSwatchData::SaveDefaultGlobalGroup()
	/// @see ColorSwatchData::LoadGlobalGroup() / ColorSwatchData::SaveGlobalGroup() / ColorSwatchData::LoadDefaultGlobalGroup() / ColorSwatchData::SaveDefaultGlobalGroup()
	/// @return												The global group. Pointer is guaranteed to be valid.
	//----------------------------------------------------------------------------------------
	ColorSwatchGroup* GetGlobalGroup();

	//----------------------------------------------------------------------------------------
	/// Replaces global group.
	/// @param[in] group							The group to copy from.
	/// @return												True on success.
	//----------------------------------------------------------------------------------------
	Bool SetGlobalGroup(ColorSwatchGroup* group);

	//----------------------------------------------------------------------------------------
	/// Loads the document-based groups from the given BaseDocument
	/// @param[in] doc								The given BaseDocument.
	/// @param[in] merge							If true the colors of the given document are merged with the stored colors. Otherwise the existing colors are discarded.
	/// @return												@formatConstant{True} if success.
	//----------------------------------------------------------------------------------------
	...
	/// @param[in] doc								The BaseDocument to store the color groups.
	/// @return												@formatConstant{True} if success.
	//----------------------------------------------------------------------------------------
	Bool Save(BaseDocument* doc);

	//----------------------------------------------------------------------------------------
	/// Loads the document-based swatch groups of the first preset with given name found in the user's Color Swatch Preset directory, including subdirectories @see GetPresetDirectory()
	/// @param[in] name								The preset name to load.
	/// @param[in] merge							Set to true to merge the preset data with current data, set to false to replace data.
	/// @return												True on success.
	//----------------------------------------------------------------------------------------
	...
	/// @param[in] forceOverwrite			Set to true to force overwrite the preset in case it already exists.
	...
	//----------------------------------------------------------------------------------------
	/// Saves the document-based groups as a Color Swatch preset.
	/// @param[in] url								The url to save preset to. Must include the preset name, and must point to an existing library. Example: SDKBrowserURL presetToSaveURL = ColorSwatchData::GetPresetDirectory() + "MyPreset"
	/// @param[in] author							The preset author.
	/// @param[in] info								Preset additional info.
	/// @param[in] forceOverwrite			Set to true to force overwrite the preset in case it already exists.
	/// @return												True if data was saved.
	...
	/// @param[in] name								The preset name.
	/// @param[in] urls								Optionally set here a pointer to array that will be filled with all urls pointing to a color swatch preset with the given name.
	/// @return												True if any preset with the given name found.
	//----------------------------------------------------------------------------------------
	...
	/// @return												True if URL points to a valid Color Swatch preset.
	//----------------------------------------------------------------------------------------
	...
	/// @param[in] mergeGlobalColors	If true the global colors will be merged inside the global group.
	/// @return												True if success.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Returns the document-based group with the given index.
	/// @param[in] index							The index of the color group. Must be 0 <= index < GetGroupCount().
	...
	/// @return												True if success.
	//----------------------------------------------------------------------------------------
	Bool SetGroupAtIndex(Int index, ColorSwatchGroup* group);
	...
	/// @param[in] colors							Colors to fill the group with.
	/// @return												The new Swatch Group or @formatConstant{nullptr}. Swatch data owns the pointed object.
	//----------------------------------------------------------------------------------------
...
{
	ColorSwatchGroup*		(*Alloc)(const String& name, Bool selected, const maxon::BaseArray<Vector>& colors);
	...
	Bool								(iColorSwatchGroup::*GetColor)(Int index, Vector& color, Bool* selected) const;
	Vector*							(iColorSwatchGroup::*GetColorEditable)(Int index, Bool* selected);
	Bool								(iColorSwatchGroup::*GetColors)(maxon::BaseArray<Vector>& colors, Bool selectedOnly) const;
	Bool								(iColorSwatchGroup::*SetColor)(Int index, const Vector& color, Int selected);
	Int									(iColorSwatchGroup::*AddColor)(const Vector& color, Bool selected, Int insertAt);
	Int									(iColorSwatchGroup::*AddColors)(const maxon::BaseArray<Vector>& colors, Bool selected, Bool merge, Int insertAt);
	void								(iColorSwatchGroup::*Reset)();
	Bool								(iColorSwatchGroup::*RemoveColor)(Int index, Int removeCount);
	void								(iColorSwatchGroup::*RemoveSelectedColors)();
	Bool								(iColorSwatchGroup::*HasDuplicatedColors)();
...
struct ColorSwatchDataLibrary : public C4DLibrary
{
	...
	Bool								(iColorSwatchData::*LoadGlobalGroup)();
	Bool								(iColorSwatchData::*SaveGlobalGroup)();
	Bool								(iColorSwatchData::*LoadDefaultGlobalGroup)(Bool ignoreFile);
	Bool								(iColorSwatchData::*SaveDefaultGlobalGroup)();
	ColorSwatchGroup*		(iColorSwatchData::*GetGlobalGroup)();
	Bool								(iColorSwatchData::*SetGlobalGroup)(ColorSwatchGroup* group);
	Bool								(iColorSwatchData::*Load)(BaseDocument* doc, Bool merge);
	Bool								(iColorSwatchData::*Save)(BaseDocument* doc);
	...
	Int									(iColorSwatchData::*GetGroupCount)() const;
	ColorSwatchGroup*		(iColorSwatchData::*GetGroupAtIndex)(Int index);
	Bool								(iColorSwatchData::*SetGroupAtIndex)(Int index, ColorSwatchGroup* group);
	ColorSwatchGroup*		(iColorSwatchData::*AddGroup)(const String& name, Bool selected, Int insertAt, const maxon::BaseArray<Vector>& colors);
	Bool								(iColorSwatchData::*InsertGroup)(ColorSwatchGroup* group, Int insertAt);
	Bool								(iColorSwatchData::*RemoveGroup)(Int index);
	void								(iColorSwatchData::*RemoveSelectedItems)();
