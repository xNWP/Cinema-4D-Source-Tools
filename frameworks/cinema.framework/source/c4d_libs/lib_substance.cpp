...
SUBSTANCE_IMPORT_RESULT ImportSubstance(BaseDocument* doc, const Filename& fn, SUBSTANCE_IMPORT_COPY& copyFile, Bool errPopup, Bool addUndo, Bool noMaterial, BaseList2D** assetPtr)
{
	...
	return lib->ImportSubstance(doc, fn, copyFile, errPopup, addUndo, noMaterial, assetPtr);
}
...
void GetSubstances(BaseDocument* const doc, AtomArray* arr, Bool selected)
{
	...
	if (!lib || !lib->GetSubstances)
		return;
	lib->GetSubstances(doc, arr, selected);
	return;
}
...
void* GetSubstanceOutput(BaseList2D* const asset, void* const graph, void* const prevOutput, UInt32& outputUid, UInt32& type, String& name, BaseBitmap** bmpPtr)
{
	...
	if (asset->GetType() != ID_SUBSTANCE_ASSET)
		return nullptr;
	return lib->SubstanceGetOutput(asset, graph, prevOutput, outputUid, type, name, bmpPtr);
}
