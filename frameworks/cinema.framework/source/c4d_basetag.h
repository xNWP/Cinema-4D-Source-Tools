...
//----------------------------------------------------------------------------------------
/// Vertex Color Tag implementation
/// VertexColorTag is a Variable Tag used to store colors (RGBA) in to the mesh structure
/// It can store data in 2 ways: per polygon vertex and per mesh vertex.
/// Per polygon vertex mode it is very similar to how UVWTag or NormalTag store data. Have to be created using Alloc with count == PolygonObject::GetPolygonCount().
/// Per mesh vertex store a single color value for mesh vertex and can beused even on object without any polygon. Have to be created using Alloc with count == PointObject::GetPointCount().
///	Using Per polygon vertex mode VertexColorTag provides all methods to set/get color per polygons or per vertex(average),
/// it also provide some convenience functions to get/ set just color or just alpha value in both modes.
...
public:
	...
	//----------------------------------------------------------------------------------------
	/// Get the read-only data handle, it automatically reconcnize if the tag is per vertex or per polygon vertex.
	/// @return												The handle to the read-only VertexColor data. The tag owns the pointed data.
	//----------------------------------------------------------------------------------------
	ConstVertexColorHandle GetDataAddressR(void);

	//----------------------------------------------------------------------------------------
	/// Get the writable data handle, it automatically reconcnize if the tag is per vertex or per polygon vertex.
	/// @return												The handle to the writable VertexColor data. The tag owns the pointed data.
	//----------------------------------------------------------------------------------------
	VertexColorHandle	GetDataAddressW(void);

	//----------------------------------------------------------------------------------------
	/// Get if the data is stored per vertex or per polygon vertex.
	/// @return												True if colors are stored per vertex otherwise false.
	//----------------------------------------------------------------------------------------
	Bool IsPerPointColor();

	//----------------------------------------------------------------------------------------
	/// Set if the data is stored per vertex or per polygon vertex.
	/// the item count is automatically updated and the data converted if possible.
	/// NOTE: all handles are invalidated after this call.
	/// @param[in] perPointColor			If true the data will be sored per vertex otherwise per polygon vertex.
	/// @return												True if colors are stored per vertex otherwise false.
	//----------------------------------------------------------------------------------------
	Bool SetPerPointMode(Bool perPointColor);

	//----------------------------------------------------------------------------------------
	/// Get Vertex Colors for the polygon i, in per polygon vertex mode.
	/// @param[in] dataptr						The data handle.
	/// @param[in] i									The polygon index.
	/// @param[out] res								The resulting VertexColorStruct.
	/// Example:
	/// @code
	...
	//----------------------------------------------------------------------------------------
	/// Set Vertex Colors for the polygon i, in per polygon vertex mode.
	/// @param[in] dataptr						The data handle.
	/// @param[in] i									The polygon index.
	/// @param[in] s									Passed VertexColorStruct to be set to the polygon.
	///
	/// In this example you can find how to get/set Data by using handles:
	/// @code
	...
	///	{
	///		VertexColorTag::Get(dataptr, faces[i], vcs);
	///		// do something with vcs (you can write values)
	///		for (Int32 a = 0; a < 4; ++a)
	///			vcs[a] = Vecor4d32(1, 0, 0, 0.5);
	///		// don't forget to set values again
	...
	//----------------------------------------------------------------------------------------
	/// Get VertexColor (RGBA) in vertex pIndex by traversing all polygons attached to it,
	/// if different values are found the average value will be returned.
	/// @param[in] dataptr						The data handle.
	/// @param[in] nb									An initialized Neighbor class, can be initialized on a polygon selection to limit the function to a specific object part.
	/// @param[in] vadr								The object CPolygon array.
	/// @param[in] pIndex							The vertex index.
	/// @return												The RGBA color for the passed vertex or the average color if multiple values.
	//----------------------------------------------------------------------------------------
	static Vector4d32 Get(ConstVertexColorHandle dataptr, Neighbor* nb, const CPolygon* vadr, Int32 pIndex);

	//----------------------------------------------------------------------------------------
	/// Set VertexColor (RGBA) in vertex pIndex if in per polygon vertex mode it traverse all polygons attached to it.
	/// @param[in] dataptr						The data handle.
	/// @param[in] nb									An initialized Neighbor class, can be initialized on a polygon selection to limit the function to a specific object part, pass nullptr in per vertex mode.
	/// @param[in] vadr								The object CPolygon array, pass nullptr in per vertex mode.
	/// @param[in] pIndex							The vertex index.
	...
	//----------------------------------------------------------------------------------------
	/// Get VertexColor (RGB) in vertex pIndex if in per polygon vertex mode it traverse all polygons attached to it.
	/// in per polygon vertex mode if different values are found the average value will be returned.
	/// @param[in] dataptr						The data handle.
	/// @param[in] nb									An initialized Neighbor class, can be initialized on a polygon selection to limit the function to a specific object part, pass nullptr in per vertex mode.
	/// @param[in] vadr								The object CPolygon array, pass nullptr in per vertex mode.
	/// @param[in] pIndex							The vertex index.
	/// @return												The RGB color for the passed vertex or the average color if multiple values.
	//----------------------------------------------------------------------------------------
	static Vector32 GetColor(ConstVertexColorHandle dataptr, Neighbor* nb, const CPolygon* vadr, Int32 pIndex);

	//----------------------------------------------------------------------------------------
	/// Set VertexColor (RGB) in vertex pIndex if in per polygon vertex mode it traverse all polygons attached to it.
	/// @param[in] dataptr						The data handle.
	/// @param[in] nb									An initialized Neighbor class, can be initialized on a polygon selection to limit the function to a specific object part, pass nullptr in per vertex mode.
	/// @param[in] vadr								The object CPolygon array, pass nullptr in per vertex mode..
	/// @param[in] pIndex							The vertex index.
	/// @param[in] color							The new RGB value to be assigned to the vertex.
	//----------------------------------------------------------------------------------------
	static void SetColor(VertexColorHandle dataptr, Neighbor* nb, const CPolygon* vadr, Int32 pIndex, const Vector32& color);

	//----------------------------------------------------------------------------------------
	/// Get VertexColor alpha value in vertex pIndex if in per polygon vertex mode it traverse all polygons attached to it
	/// in per polygon vertex mode if different values are found the average value will be returned.
	/// @param[in] dataptr						The data handle.
	/// @param[in] nb									An initialized Neighbor class, can be initialized on a polygon selection to limit the function to a specific object part, pass nullptr in per vertex mode.
	/// @param[in] vadr								The object CPolygon array, pass nullptr in per vertex mode.
	/// @param[in] pIndex							The vertex index.
	/// @return												The alpha value for the passed vertex or the average alpha value if multiple values.
	//----------------------------------------------------------------------------------------
	static Float32 GetAlpha(ConstVertexColorHandle dataptr, Neighbor* nb, const CPolygon* vadr, Int32 pIndex);

	//----------------------------------------------------------------------------------------
	/// Set VertexColor alpha in vertex pIndex if in per polygon vertex mode it traverse all polygons attached to it
	/// @param[in] dataptr						The data handle.
	/// @param[in] nb									An initialized Neighbor class, can be initialized on a polygon selection to limit the function to a specific object part, pass nullptr in per vertex mode.
	/// @param[in] vadr								The object CPolygon array, pass nullptr in per vertex mode.
	/// @param[in] pIndex							The vertex index.
	...
	//----------------------------------------------------------------------------------------
	/// Allocate a new VertexColorTag. Destroy the allocated VertexColorTag tag with Free().
	/// @param[in] count							The element count in the tag, must be == to the object polygon count in case of per polygon vertex storeage other wise muse bee == o the object point count.
	/// @return												The allocated VertexColorTag tag, or nullptr if the allocation failed.
	//----------------------------------------------------------------------------------------
	static VertexColorTag* Alloc(Int32 count);

	//----------------------------------------------------------------------------------------
	/// Destructs VertexColorTag tags allocated with Alloc().
	/// @param[in] bl									The VertexColorTag tag to destruct. If the pointer is nullptr nothing happens. The pointer is assigned nullptr afterward
	//----------------------------------------------------------------------------------------
