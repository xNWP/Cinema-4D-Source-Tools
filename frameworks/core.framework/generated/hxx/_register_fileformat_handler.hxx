...
namespace maxon
{
	...
	const maxon::Char* const FileFormatHandlerInterface::MTable::_ids = 
		...
		"GetHandlerType@8203984a32669b92\0" // Result<DataType> GetHandlerType() const
		"CreateHandler@11eb1c35ba4964cc\0" // Result<ObjectRef> CreateHandler(const Url& url) const
		"GetDependends@7849c50e98fbe867\0" // FILEFORMAT_PRIORITY GetDependends() const
	"";
	PRIVATE_MAXON_INTERFACE_REGISTER_VIRTUAL_SINGLE_BASE(FileFormatHandlerInterface, , "net.maxon.interface.fileformathandler", "maxon.FileFormatHandlerInterface", (&ObjectInterface::_interface));
...
#ifndef MAXON_COMPILER_CLANG
#endif
		"DetectAll@f0906a41dadcc17f\0" // Result<BaseArray<FileFormatHandler>> DetectAll(const Url& url, const Block<const DataType>& limitDetection, const DetectedCallback& detectedCallback)
	"";
	MAXON_WARNING_PUSH
	...
	maxon::Bool FileFormatDetectionInterface::MTable::InitUnresolved(maxon::NonvirtualInterfaceReference::UnresolvedAssembler assembler, MTable* tbl)
	{
		...
		if (maxon::details::NullReturnType<Result<BaseArray<FileFormatHandler>>>::value == maxon::NULL_RETURN_TYPE::OTHER) tbl->_FileFormatDetectionInterface_DetectAll = &Hxx2::Wrapper<Hxx2::Unresolved>::_FileFormatDetectionInterface_DetectAll;
	#else
		tbl->_FileFormatDetectionInterface_DetectAll = &Hxx2::Wrapper<Hxx2::Unresolved>::_FileFormatDetectionInterface_DetectAll;
	#endif
	#ifdef PRIVATE_MAXON_ASSEMBLE_UNRESOLVED
