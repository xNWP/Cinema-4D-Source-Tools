...
struct MaxonRegistry // has to be in global namespace as a globally visible default
{
	...
	static const maxon::EntityBase::FLAGS ENTRY_FLAGS = maxon::EntityBase::FLAGS::NONE;
...
protected:
	// used internally for RegistryModuleInfo
	explicit ModuleInfo(const Char* idValue) : _super(EntityBase::TYPE::REGISTRY_MODULE, Id(idValue), nullptr, nullptr, EntityBase::FLAGS::NONE), _next(nullptr), _binary(&g_maxon)
	{
	}
...
public:
	...
	template <typename T, typename REG> ObjectDefinition(LiteralId&& iid, Result<T> (*init) (MAXON_SOURCE_LOCATION_DECLARATION, Id&), REG* dummy, TranslationUnit* unit, const Char* file, EntityBase::FLAGS flags)
		: _super(maxon::details::EntityType<T>::value, std::move(iid), unit, file, flags | REG::ENTRY_FLAGS),
...
private:
	...
	template <typename T> static Result<void> InitObject(ObjectDefinition* this_)
	{
		...
		if (std::is_same<const typename Dereference<T>::type, const ClassInterface>::value)
		{
...
public:
	...
	explicit Global(Result<void>(*init)(), TranslationUnit& unit = g_translationUnit)
		: _privateSuper(
#ifdef __BASE_FILE__
			__BASE_FILE__,
#else
			"global",
#endif
			&PrivateGlobalCallInit, &PrivateGlobalFree, &unit), _init(init)
	{
	}
...
protected:
	...
	static Result<void> PrivateGlobalCallInit(maxon::details::GlobalBase* global)
	{
		return PrivateGlobalGet(global)->_init();
	}
...
#define PRIVATE_MAXON_DECLARATION_REGISTER_1(cnt, Name) \
	MAXON_UNIT_REGISTER_CHECK("MAXON_DECLARATION_REGISTER"); \
