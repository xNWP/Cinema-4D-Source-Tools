...
public:
	...
	/// @MAXON_ANNOTATION{incompleteReturnType=true}
	//----------------------------------------------------------------------------------------
...
public:
	...
	Result<typename std::conditional<std::is_void<T>::value, typename IsFidClass<KEY>::type, T>::type> Get(KEY&& key) const
	{
...
// TODO: (Sven) - CR -  must this be done before RC?
// TODO: (Sven) Support a generic error String (or remove)
MAXON_ATTRIBUTE_FORCE_INLINE const Url& operator %(const Url& ref, ThreadReferencedError& err)
{
	if (ref.GetPointer())
	{
		err = nullptr;
...
MAXON_ATTRIBUTE_FORCE_INLINE Url& operator %(Url& ref, ThreadReferencedError& err)
{
	if (ref.GetPointer())
	{
		err = nullptr;
