...
#endif
{
	...
	/// @return												*this.
	//----------------------------------------------------------------------------------------
	Vec3& operator =(const typename std::conditional<STRIDE == 1, DummyParamType, Vec3>::type& src)
	{
		x = src.x;
		y = src.y;
		z = src.z;
	...
	/// Adds two vectors
	template <typename T2, Int S2> Vec3& operator +=(const Vec3<T2, S2>& v)
	{
		x += v.x;
		y += v.y;
		z += v.z;
	...
	/// Multiplies each vector component by a scalar
	friend Unstrided operator *(ValueTypeParam s, const Vec3& v)
	{
	...
	/// Multiplies each vector component by a scalar
	template <typename S> Vec3<typename MultiplicativePromotion<T, S, std::is_scalar<S>::value>::type, 1> operator *(S s) const
	{
	...
	//----------------------------------------------------------------------------------------
	template <typename T2, Int S2> BoolType operator ==(const Vec3<T2, S2>& v) const
	{
	...
	friend T Dot(const Vec3& v1, const Vec3& v2)
	{
	...
	/// Calculates dot product of v1 and v2
	template <typename T2, Int S2> friend typename MultiplicativePromotion<T, T2>::type Dot(const Vec3& v1, const Vec3<T2, S2>& v2)
	{
		return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
	}
	...
	friend Unstrided Min(const Vec3& a, const Vec3& other)
	{
	...
	friend Unstrided Max(const Vec3& a, const Vec3& other)
	{
		return Unstrided(a.x > other.x ? a.x : other.x, a.y > other.y ? a.y : other.y, a.z > other.z ? a.z : other.z); 
	}
	...
	/// Set the minimum of each component.
	friend void SetMin(Vec3& a, const Vec3& other)
	{
		a.ClampMax(other);
	}

	/// Set the maximum of each component.
	friend void SetMax(Vec3& a, const Vec3& other)
	{
		a.ClampMin(other);
	}
	...
	friend T GetAngle(const Vec3& v1, const Vec3& v2)
	{
	...
	const Col3<T, STRIDE>& GetColor() const { return *(const Col3<T, STRIDE>*) this; }
	Col3<T, STRIDE>& GetColor() { return *(Col3<T, STRIDE>*) this; }

	/// Calculates cross product of v1 and v2
	friend Unstrided Cross(const Vec3& v1, const Vec3& v2)
	{
		return Unstrided(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x);
	}

	/// Calculates cross product of v1 and v2
	template <typename T2, Int S2> friend Vec3<typename MultiplicativePromotion<T, T2>::type, 1> Cross(const Vec3& v1, const Vec3<T2, S2>& v2)
	{
	...
	template <typename T2> Vec3& operator *=(const Mat3<T2>& m)
	{
...
#ifndef STRIDE
/// Multiplies each vector component by a scalar
template <typename T, Int STR, typename S> inline Vec3<typename MultiplicativePromotion<T, S, std::is_scalar<S>::value>::type, 1> operator *(S s, const Vec3<T, STR>& v)
{
	return Vec3<typename MultiplicativePromotion<T, S>::type, 1>(v.x * s, v.y * s, v.z * s);
}

template <GET_DATATYPE_POLICY POLICY, typename T> inline Result<DataType> PrivateGetDataType(Vec3<T, 1>**, OverloadRank0)
{
	iferr_scope;
