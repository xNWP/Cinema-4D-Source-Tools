...
#endif
{
	...
	const Col4<T, STRIDE>& GetColor() const { return *(const Col4<T, STRIDE>*) this; }

	/// Reinterprets the vector as color.
	Col4<T, STRIDE>& GetColor() { return *(Col4<T, STRIDE>*) this; }

	/// Gets the x/y/z components without w.
	const Vec3<T, STRIDE>& GetVector3() const { return *(const Vec3<T, STRIDE>*) this; }
	...
	MAXON_IMPLICIT Vec4(const typename std::conditional<STRIDE == 1, DummyParamType, Vec4>::type& src) : MAXON_MOVE_MEMBERS(x, y, z, w)
	{
	}
	...
	/// Multiplies each vector component by a scalar
	friend Unstrided operator *(ValueTypeParam s, const Vec4& v)
	{
	...
	/// Multiplies each vector component by a scalar
	template <typename S> Vec4<typename MultiplicativePromotion<T, S, std::is_scalar<S>::value>::type, 1> operator *(S s) const
	{
	...
	/// Multiplies two vectors component-wise
	Unstrided operator *(const Vec4& v) const
	{
	...
	//----------------------------------------------------------------------------------------
	template <typename T2, Int S2> BoolType operator ==(const Vec4<T2, S2>& v) const
	{
	...
	/// @return												True if this and v are equal, otherwise false.
	//----------------------------------------------------------------------------------------
	BoolType operator ==(const Vec4& v) const
	{
		return x == v.x && y == v.y && z == v.z && w == v.w;
	}
	...
	// is not necessarily true.
 	BoolType LessThanOrEqual(const Vec4& v) const
 	{
 		return x <= v.x && y <= v.y && z <= v.z && w <= v.w;
 	}
	...
	friend T Dot(const Vec4& v1, const Vec4& v2)
	{
	...
	/// Calculates dot product of v1 and v2
	template <typename T2, Int S2> friend typename MultiplicativePromotion<T, T2>::type Dot(const Vec4& v1, const Vec4<T2, S2>& v2)
	{
		return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z + v1.w * v2.w;
	}
	...
	friend Unstrided Min(const Vec4& a, const Vec4& other)
	{
	...
	friend Unstrided Max(const Vec4& a, const Vec4& other)
	{
		return Unstrided(a.x > other.x ? a.x : other.x, a.y > other.y ? a.y : other.y, a.z > other.z ? a.z : other.z, a.w > other.w ? a.w : other.w); 
	}
	...
	//----------------------------------------------------------------------------------------
	Vec4 GetRightRotated(Int rots) const
	{
		switch (rots & 3)
		{
			case 0:
...
#ifndef STRIDE
/// Multiplies each vector component by a scalar
template <typename T, Int STR, typename S> inline Vec4<typename MultiplicativePromotion<T, S, std::is_scalar<S>::value>::type, 1> operator *(S s, const Vec4<T, STR>& v)
{
	return Vec4<typename MultiplicativePromotion<T, S>::type, 1>(v.x * s, v.y * s, v.z * s, v.w * s);
}

template <GET_DATATYPE_POLICY POLICY, typename T> inline Result<DataType> PrivateGetDataType(Vec4<T, 1>**, OverloadRank0)
{
	iferr_scope;
