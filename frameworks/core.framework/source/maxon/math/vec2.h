...
#endif
{
	...
	/// Multiplies each vector component by a scalar
	template <typename S> Vec2<typename MultiplicativePromotion<T, S, std::is_scalar<S>::value>::type, 1> operator *(S s) const
	{
...
#ifndef STRIDE
/// Multiplies each vector component by a scalar
template <typename T, Int STR, typename S> inline Vec2<typename MultiplicativePromotion<T, S, std::is_scalar<S>::value>::type, 1> operator *(S s, const Vec2<T, STR>& v)
{
	return Vec2<typename MultiplicativePromotion<T, S>::type, 1>(v.x * s, v.y * s);
}

template <GET_DATATYPE_POLICY POLICY, typename T> inline Result<DataType> PrivateGetDataType(Vec2<T, 1>**, OverloadRank0)
{
	iferr_scope;
