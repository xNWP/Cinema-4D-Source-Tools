...
{
public:
	ResultOk() : _value(&NullValue<T&>()) { }

	MAXON_IMPLICIT ResultOk(T& value) : _value(&value) { }
...
public:
	...
	MAXON_IMPLICIT ResultMemT(ERROR_OK) : _value(true) { static_assert(std::is_same<T, Bool>::value, "Constructor only allowed for ResultMemT<Bool>."); }

	MAXON_IMPLICIT ResultMemT(ResultOk<void>) : _value(true) { static_assert(std::is_same<T, Bool>::value, "Constructor only allowed for ResultMemT<Bool>."); }
	...
	template <typename T2, typename = typename std::enable_if<std::is_constructible<T, T2*>::value && std::is_pointer<T>::value>::type> MAXON_IMPLICIT ResultMemT(const ResultMemT<T2*>& result) : _value(result.GetValue()) { }
	template <typename T2, typename = typename std::enable_if<std::is_constructible<T, T2*>::value && std::is_pointer<T>::value>::type> MAXON_IMPLICIT ResultMemT(ResultMemT<T2*>&& result) : _value(std::move(result.GetValue())) { }

	// Enables conversion of inherited types, e.g. from ResultMemT<ArrayMap::Iterator> to ResultMemT<HybridMap::Iterator>.
	template <typename T2, typename = typename std::enable_if<std::is_constructible<T, T2>::value && std::is_pointer<T>::value == std::is_pointer<T2>::value>::type> MAXON_IMPLICIT ResultMemT(const ResultMemT<T2>& result) : _value(result.GetValue()) { }
	template <typename T2, typename = typename std::enable_if<std::is_constructible<T, T2>::value && std::is_pointer<T>::value == std::is_pointer<T2>::value>::type> MAXON_IMPLICIT ResultMemT(ResultMemT<T2>&& result) : _value(std::move(result.GetValue())) { }

	template <typename T2> MAXON_IMPLICIT ResultMemT(const ResultRef<T2>& result) : _value(result == OK) { static_assert(std::is_same<T, Bool>::value, "Constructor only allowed for Bool."); }
	...
	/// @return												Result value.
	//----------------------------------------------------------------------------------------
	T GetPointer() const { static_assert(std::is_pointer<T>::value, "Must be a pointer type"); return _value; }
...
public:
	...
	MAXON_IMPLICIT ResultRef(T& value) : _value(&value) { }
	...
	template <typename T2, typename = typename std::enable_if<(std::is_same<const T, const T2>::value || std::is_base_of<T, T2>::value) && (std::is_const<T>::value >= std::is_const<T2>::value)>::type>
...
protected:
	...
	template <typename T2> using ConstructibleFrom = std::enable_if<std::is_constructible<T, T2>::value>;

public:
	static const Bool HAS_ERROR = true;
...
public:
	...
	template <typename T2> using ConstructibleFrom = std::enable_if<std::is_same<T&, T2>::value || std::is_same<typename std::remove_const<T>::type&, T2>::value>;
	static const Bool HAS_ERROR = true;

	ResultBase() : _value(std::is_const<T>::value ? (T*) &NullValue<typename std::conditional<std::is_const<T>::value, T&, const Int&>::type>() : const_cast<T*>(&GetZeroRef<T>())) { _error._error = nullptr; }

	MAXON_IMPLICIT ResultBase(CopyType value) : _value(&value) { _error._error = nullptr; }

	T& GetValue() const { DebugAssert(!_error._error); return *_value; }

	T& GetValueAndError(ThreadReferencedError& error) const { error = _error; return *_value; }

	T& GetValueAndError(Error& error) const { PrivateSetError(error, _error._error); return *_value; }

	const ThreadReferencedError& GetErrorStorage() const { return _error; }

	ThreadReferencedError& GetErrorStorage() { return _error; }
...
public:
	...
	void GetValue() const { DebugAssert(!_error._error); }

	void GetValueAndError(ThreadReferencedError& error) const { error = _error; }
...
template <typename T> class PrivateIsErrorClassHelper<T, true> : public std::remove_reference<T>::type::DirectlyReferencedType::HasBase::template Check<ErrorInterface>
{
};
...
public:
	...
	ResultSuper() PRIVATE_MAXON_DEFAULT_CTOR;
...
{
public:
	static_assert(!std::is_rvalue_reference<RESULT_TYPE>::value, "Can't instantiate Result template with an r-value reference type.");
	...
	#define PRIVATE_MAXON_ENABLE_IF_ERROR(TYPE) typename std::enable_if<std::remove_reference<E>::type::DirectlyReferencedType::HasBase::template Check<ErrorInterface>::value, TYPE>::type
#else
	#define PRIVATE_MAXON_ENABLE_IF_ERROR(TYPE) typename std::enable_if<std::remove_reference<E>::type::DirectlyReferencedType::HasBase::template Check<ErrorInterface>::value && MAXON_IS_COW_KIND(std::remove_reference<E>::type::Handler::KIND), TYPE>::type
#endif
...
#define PRIVATE_MAXON_ENABLE_IF_ERROR_DUMMY PRIVATE_MAXON_ENABLE_IF_ERROR(DummyParamType*) = (DummyParamType*) nullptr
	...
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT Result(typename std::conditional<std::is_void<RESULT_TYPE>::value, ResultMem, const volatile DummyParamType&>::type value) : Super()
	{
		if (value == FAILED)
	...
	/// @param[in] value							The ResultOk from which the value is taken.
	//----------------------------------------------------------------------------------------
	template <typename T2, typename = typename Super::template ConstructibleFrom<T2>::type> MAXON_IMPLICIT Result(ResultOk<T2>&& value) : Super(std::move(value.GetValue()))
	{
	}
	...
	/// @param[in] value							The ResultOk from which the value is taken.
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT Result(typename std::conditional<std::is_void<RESULT_TYPE>::value, ResultOk<RESULT_TYPE>, DummyParamType&&>::type value) : Super()
	{
	}
	...
	//----------------------------------------------------------------------------------------
	template <typename T2> MAXON_IMPLICIT Result(const ResultMemT<T2>& value, typename std::enable_if<std::is_void<typename SFINAEHelper<RESULT_TYPE, T2>::type>::value, DummyParamType*>::type = (DummyParamType*) nullptr) : Super()
	{
	...
	template <typename T2, typename = typename Super::template ConstructibleFrom<T2>::type> MAXON_IMPLICIT Result(ResultMemT<T2>&& value)
	{
		if (MAXON_UNLIKELY(value == FAILED))
			...
			this->_value = std::move(value.GetValue());
	}
	...
	/// @param[in] value							If nullptr, set the error object to an OutOfMemoryError.
	//----------------------------------------------------------------------------------------
	template <typename T2> MAXON_IMPLICIT Result(const ResultRef<T2>& value, typename std::enable_if<std::is_void<typename SFINAEHelper<RESULT_TYPE, T2>::type>::value, DummyParamType*>::type = (DummyParamType*) nullptr) : Super()
	{
		if (!value.GetPointer())
			this->_error = CreateErrorPtr(MAXON_SOURCE_LOCATION, ERROR_TYPE::OUT_OF_MEMORY);
	}
	...
	/// for a general <tt>Result<T></tt>.
	//----------------------------------------------------------------------------------------
	MAXON_IMPLICIT Result(typename std::conditional<std::is_void<RESULT_TYPE>::value, ERROR_OK, volatile DummyParamType&>::type) : Super() { }
	...
	/// for a general <tt>Result<T></tt>.
	//----------------------------------------------------------------------------------------
	template <typename T2> Result(const Result<T2>& src, typename std::enable_if<std::is_void<typename SFINAEHelper<RESULT_TYPE, T2>::type>::value, DummyParamType*>::type = (DummyParamType*) nullptr) : Super() { this->_error = src.GetErrorStorage(); }
	...
	/// using the value and/or error of @p src.
	//----------------------------------------------------------------------------------------
	template <typename T2, typename = typename Super::template ConstructibleFrom<T2>::type> MAXON_IMPLICIT Result(Result<T2>&& src) : Super(std::move(src.UncheckedGetValue())) { this->_error = src.GetErrorStorage(); }
	...
	explicit Result(typename std::conditional<std::is_same<typename std::decay<RESULT_TYPE>::type, Bool>::value || std::is_void<RESULT_TYPE>::value, volatile DummyParamType&&, Bool>::type) = delete;
	MAXON_IMPLICIT Result(typename std::conditional<std::is_void<RESULT_TYPE>::value, Bool, const volatile DummyParamType&&>::type) = delete;
#else
	explicit Result(typename std::conditional<std::is_same<typename std::decay<RESULT_TYPE>::type, Bool>::value, volatile DummyParamType&&, Bool>::type) = delete;
#endif
	...
	//----------------------------------------------------------------------------------------
	const Error& GetError() const { return *(this->_error._error ? this->_error._error : reinterpret_cast<const Error*>(&this->_error)); }
	...
	void operator =(typename std::conditional<std::is_reference<RESULT_TYPE>::value, RESULT_TYPE, const DummyParamType&>::type) = delete;
...
template <typename T> MAXON_ATTRIBUTE_FORCE_INLINE T& operator %(ResultRef<T> ptr, ThreadReferencedError& err)
{
	...
	else
	{
		// Use CreateErrorPtr() because the compiler can omit the call if the err isn't used.
		err = CreateErrorPtr(MAXON_SOURCE_LOCATION, ERROR_TYPE::OUT_OF_MEMORY);
		return std::is_const<T>::value ? (T&) NullValue<typename std::conditional<std::is_const<T>::value, T&, const Int&>::type>() : const_cast<T&>(GetZeroRef<T>());
	}
}
