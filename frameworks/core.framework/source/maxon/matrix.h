...
template <typename V> struct SqMat3
{
	...
	explicit SqMat3(ENUM_DONT_INITIALIZE v) : v1(maxon::DONT_INITIALIZE), v2(maxon::DONT_INITIALIZE), v3(maxon::DONT_INITIALIZE) { }
	...
	{
		return SqMat3<typename MultiplicativePromotion<V, typename V2::ValueType>::type>(v1 * m2.v1.x + v2 * m2.v1.y + v3 * m2.v1.z,
	...
	{
		return SqMat3(s * m.v1, s * m.v2, s * m.v3);
	}

	/// Scales all matrix components by a scalar value
	SqMat3 operator *(ValueTypeParam s) const
	{
		return SqMat3(v1 * s, v2 * s, v3 * s);
	}
	...
	SqMat3 operator /(ValueTypeParam s) const
	{
		...
		return SqMat3(v1 * is, v2 * is, v3 * is);
	}

	/// Adds two matrices component-wise
	SqMat3 operator +(const SqMat3& m2) const
	{
		return SqMat3(v1 + m2.v1, v2 + m2.v2, v3 + m2.v3);
	}
	...
	SqMat3 operator ~() const
	{
		...
		det = ValueType(1) / det;
		return SqMat3(V((v2.y * v3.z - v3.y * v2.z) * det,
	...
	{
		return SqMat3(v1.GetNormalized(), v2.GetNormalized(), v3.GetNormalized());
	}

	/// Returns the transposed matrix.
	SqMat3 GetTransposed() const
	{
		return SqMat3(V(v1.x, v2.x, v3.x), V(v1.y, v2.y, v3.y), V(v1.z, v2.z, v3.z));
	}
	...
	{
		return SqMat3(V(v3.z * v2.y - v3.y * v2.z, v3.x * v2.z - v3.z * v2.x, v3.y * v2.x - v3.x * v2.y),
	...
	{
		return MAXON_HASHCODE(v1, v2, v3);
	}
...
template <typename V> struct Mat3
{
	...
	/// The rule is m1 AFTER m2
	/// If you transform a point with the result matrix this is identical to first transforming with m2 and then with m1
	template <typename V2> Mat3<typename MultiplicativePromotion<V, typename V2::ValueType>::type> operator *(const Mat3<V2>& m2) const
	{
	...
	/// Scales all matrix components by a scalar value
	friend Mat3 operator *(ValueTypeParam s, const Mat3& m)
	{
...
template <GET_DATATYPE_POLICY POLICY, typename T> inline Result<DataType> PrivateGetDataType(SqMat3<T>**, OverloadRank0)
{
	iferr_scope;
	DataType t = CheckedGetDataType<T, Or<GET_DATATYPE_POLICY, POLICY, GET_DATATYPE_POLICY::NO_DECAY>::value>() iferr_return;
	return ParametricTypes::SqMat().Instantiate(t, ToSingletonBlock(Int64(3)));
}

template <GET_DATATYPE_POLICY POLICY, typename T> inline Result<DataType> PrivateGetDataType(Mat3<T>**, OverloadRank0)
{
	iferr_scope;
	DataType t = CheckedGetDataType<T, Or<GET_DATATYPE_POLICY, POLICY, GET_DATATYPE_POLICY::NO_DECAY>::value>() iferr_return;
	Member tt[] = {t, t};
