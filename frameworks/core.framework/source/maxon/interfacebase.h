...
template <typename T, Bool HAS_NONSTATIC = T::MTable::HAS_NONSTATIC> struct PrivateInitInterfaceType
{
	...
	{
		new (info) DataTypeImpl(std::move(iid), nullptr, nullptr, (T*) nullptr, (typename InterfaceDataTypeFunctionsTrait<T>::Functions*) nullptr, VALUEKIND::NONE, impl, VALUEKIND::NONVIRTUAL_INTERFACE);
...
public:
	...
	template <typename T, typename = typename std::enable_if<!std::is_same<const T, const ObjectInterface>::value && !std::is_same<const T, const ReferencedType>::value
	...
	static const Bool DERIVED_FROM_OBJECT = !std::is_same<const ObjectInterface, const ReferencedType>::value && IsVirtualInterfaceType<ReferencedType>::value;
...
template <Bool DELETE, typename SELF, typename T, typename DST, typename SRC, Bool = !std::is_base_of<SELF, SRC>::value && maxon::HasErasedBase<T, typename SELF::ReferencedType>::value> struct ReferenceConversionHelper
{
...
public:
	...
	{
		reinterpret_cast<typename ReferenceBaseSuper::BaseRefType*>(this)->operator =(nullptr);
		return *this;
	}
...
#define MAXON_IMPLEMENTATION_REGISTER(C, ...) \
  ...
  maxon::NonvirtualInterfaceImplementation C::_Wrapper::_implementation((C*) nullptr, #C, C::Interface::_interface, (maxon::NonvirtualInterfaceReference::MTableInitializer) &maxon::details::InitMTable<typename C::Interface::MTable, typename C::_Wrapper>, C::Interface::REFERENCE_KIND | maxon::EntityBase::FLAGS(__VA_ARGS__), &maxon::g_translationUnit, __FILE__); \
	C* C::_nullValue; \
...
{
	static const Bool value = SAFE ? std::is_constructible<TO, const FROM&>::value
		: ((std::is_const<typename TO::DirectlyReferencedType>::value >= std::is_const<typename FROM::DirectlyReferencedType>::value)
			 && (maxon::details::HasCommonType<const typename TO::DirectlyReferencedType*, const typename FROM::DirectlyReferencedType*>::value
					 || (std::is_base_of<ObjectInterface, typename TO::DirectlyReferencedType>::value && std::is_base_of<ObjectInterface, typename FROM::DirectlyReferencedType>::value)));
};
