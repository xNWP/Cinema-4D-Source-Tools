...
# ifdef CPP_853_HACK // use space after # so that the source processor doesn't recognize those directives
	CPP_853_HACK_MAXON_INTERFACE_SIMPLE_VIRTUAL(VArrayInterface, MAXON_REFERENCE_COPY_ON_WRITE);
# else
...
public:
	...
	static const Bool GENERIC = std::is_same<const Generic, const TYPE>::value;
	...
	{
		return ConstIterator(*static_cast<const VArrayInterface*>(this));
	}
	...
	Iterator Begin()
	{
		return Iterator(*static_cast<VArrayInterface*>(this));
	}
	...
	{
		return ConstIterator(*static_cast<const VArrayInterface*>(this), this->GetCount());
	}
	...
	Iterator End()
	{
		return Iterator(*static_cast<VArrayInterface*>(this), this->GetCount());
	}
...
private:
	...
	{
		iferr (VArrayInterface<TYPE>* a = src.Clone(true))
			return nullptr;
		return a;
	}
};
...
public:
	...
	template <typename T> MAXON_FUNCTION void AssertType() const
	{
		...
		{
			this->GetValueDataType().template AssertType<typename std::conditional<std::is_same<Generic, T>::value, Int, T>::type>();
		}
#endif
	}
...
#pragma warning disable 597
	...
	{
		return *(ArrayInterface<typename std::conditional<std::is_same<TYPE, Generic>::value, DummyReturnType, Generic>::type>*) this;
	}
#ifdef MAXON_COMPILER_INTEL
#pragma warning enable 597
#endif
...
#endif
		: ArrayBase0<ArrayInterface<TYPE>, TYPE, VArrayInterface<TYPE>, DefaultCompare>(*src._vtable), _refDummy(src._refDummy)
	{
	}
#endif
...
private:
	...
	static ArrayInterface* Alloc(MAXON_SOURCE_LOCATION_DECLARATION, const ArrayInterface& src)
	{
		iferr (VArrayInterface<TYPE>* a = src.Clone(true))
			return nullptr;
		return static_cast<ArrayInterface*>(a);
	}
