...
Bool SMDLoaderData::Identify(BaseSceneLoader *node, const Filename &name,
                             UChar *probe, Int32 size) {
  // unfortunately SMD is an ascii format with no FourCC/Header so we will just
  // assume that the file-type is SMD based on the extension alone.
  return (name.GetSuffix().ToLower() == "smd") ? true : false;
}

FILEERROR SMDLoaderData::Load(BaseSceneLoader *node, const Filename &name,
  ...
  Name.ClearSuffix();
  if (SMDObj.Mesh)
    SMDObj.Mesh->SetName(Name.GetString());
  ...
  if (!(filterflags & SCENEFILTER::MERGESCENE) &&
      smd.SkeletonAnimation.size() > 1 && config.IncludeAnimation) {
    doc->SetMaxTime(
        BaseTime((Float)smd.SkeletonAnimation.size() - 1, doc->GetFps()));
    doc->SetLoopMaxTime(
        BaseTime((Float)smd.SkeletonAnimation.size() - 1, doc->GetFps()));
  }
...
SMDLoaderData::SMDObject SMDLoaderData::CreateSMD(const StudiomdlData &smd,
  ...
  maxon::Matrix TransformRotationMatrix =
      ...
      MatrixScale(Vector(-1.0f, 1.0f, 1.0f)) * MatrixRotX(PI / 2);
  ...
  if (SMDObj.Skeleton.empty() && config.IncludeSkeleton) {
    IF_PROFILING(
        Benchmark CreateSMDBuildSkeletonBench("CreateSMD-BuildSkeleton"));
    ...
    for (auto &bone : smd.Bones) {
      ...
      if (Entry.ParentId == -1)
        config.doc->InsertObject(Entry.Object, RootNull,
                                 RootNull->GetDownLast());
      else
        config.doc->InsertObject(
            Entry.Object, SMDObj.Skeleton[Entry.ParentId].Object,
    ...
    for (auto &frame : smd.SkeletonAnimation) {
      if (!config.IncludeAnimation && frame.Time != 0)
        break;

      for (auto &bone : frame.Entries) {
        maxon::Matrix BoneLocalMatrix = HPBToMatrix(
            maxon::Vector(bone.Rotation.x, bone.Rotation.y, bone.Rotation.z),
            ROTATIONORDER::XYZLOCAL);
        BoneLocalMatrix = ~BoneLocalMatrix;
        BoneLocalMatrix.off =
            maxon::Vector64(bone.Position.x, bone.Position.y, bone.Position.z);
        ...
        if (frame.Time == 0)
          SMDObj.Skeleton[bone.Id].Object->SetMl(BoneLocalMatrix);

        if (config.IncludeAnimation && smd.SkeletonAnimation.size() > 1) {
          CTrack *XTrack, *YTrack, *ZTrack, *XRTrack, *YRTrack, *ZRTrack;
          CCurve *XCurve, *YCurve, *ZCurve, *XRCurve, *YRCurve, *ZRCurve;

          if (frame.Time == 0) {
            XTrack = CTrack::Alloc(
                SMDObj.Skeleton[bone.Id].Object,
            ...
            XRTrack = CTrack::Alloc(
                SMDObj.Skeleton[bone.Id].Object,
                DescID(DescLevel(ID_BASEOBJECT_REL_ROTATION, DTYPE_VECTOR, 0),
                       DescLevel(VECTOR_X, DTYPE_REAL, 0)));
            YRTrack = CTrack::Alloc(
                SMDObj.Skeleton[bone.Id].Object,
                DescID(DescLevel(ID_BASEOBJECT_REL_ROTATION, DTYPE_VECTOR, 0),
                       DescLevel(VECTOR_Y, DTYPE_REAL, 0)));
            ZRTrack = CTrack::Alloc(
                SMDObj.Skeleton[bone.Id].Object,
                DescID(DescLevel(ID_BASEOBJECT_REL_ROTATION, DTYPE_VECTOR, 0),
                       DescLevel(VECTOR_Z, DTYPE_REAL, 0)));
          ...
          SMDObj.Skeleton[bone.Id].Object->GetVectorTracks(
              DescID(DescLevel(ID_BASEOBJECT_REL_POSITION, DTYPE_VECTOR, 0)),
        ...
        end_find_tracks:;
          ...
          CKey *XKey =
              XCurve->AddKey(BaseTime(frame.Time, DocumentFramesPerSecond));
          XKey->SetValue(XCurve, BoneLocalMatrix.off.x);
          CKey *YKey =
              YCurve->AddKey(BaseTime(frame.Time, DocumentFramesPerSecond));
          YKey->SetValue(YCurve, BoneLocalMatrix.off.y);
          CKey *ZKey =
              ZCurve->AddKey(BaseTime(frame.Time, DocumentFramesPerSecond));
          ZKey->SetValue(ZCurve, BoneLocalMatrix.off.z);

          maxon::Vector RotationVector =
              MatrixToHPB(BoneLocalMatrix, ROTATIONORDER::HPB);
          ...
          CKey *XRKey =
              XRCurve->AddKey(BaseTime(frame.Time, DocumentFramesPerSecond));
          XRKey->SetValue(XRCurve, RotationVector.x);
          CKey *YRKey =
              YRCurve->AddKey(BaseTime(frame.Time, DocumentFramesPerSecond));
          YRKey->SetValue(YRCurve, RotationVector.y);
          CKey *ZRKey =
              ZRCurve->AddKey(BaseTime(frame.Time, DocumentFramesPerSecond));
  ...
  if (config.IncludeMesh) {
    ...
    if (PolyCount != 0) {
      ...
      {
        IF_PROFILING(Benchmark CreateSMDBuildMeshPolyonsBench(
            "CreateSMD-BuildMesh-Polygons"));
        Int32 PolygonIterator = 0;
        for (auto &triangle : smd.Triangles) {
          maxon::Vector C(triangle.Vertices[0].Position.x,
                          triangle.Vertices[0].Position.y,
                          triangle.Vertices[0].Position.z);
          maxon::Vector B(triangle.Vertices[1].Position.x,
                          triangle.Vertices[1].Position.y,
                          triangle.Vertices[1].Position.z);
          maxon::Vector A(triangle.Vertices[2].Position.x,
          ...
          for (Int32 i = Int32(PointVec.size() - 1); i >= 0; i--) {
            ...
            if (PointVec[i] == A)
              Aindex = i;
            else if (PointVec[i] == B)
              Bindex = i;
            else if (PointVec[i] == C)
              Cindex = i;
          }
      ...
      if (config.IncludeNormals) {
        IF_PROFILING(Benchmark CreateSMDBuildMeshNormalsBench(
            "CreateSMD-BuildMesh-Normals"));
        /* Normals */
        ...
        for (Int32 i = 0; i < PolyCount; ++i) {
          ...
          VertexNormals.c =
              TransformRotationMatrix *
              maxon::Vector(smd.Triangles[i].Vertices[0].Normals.x,
                            smd.Triangles[i].Vertices[0].Normals.y,
                            smd.Triangles[i].Vertices[0].Normals.z);
          VertexNormals.b =
              TransformRotationMatrix *
              maxon::Vector(smd.Triangles[i].Vertices[1].Normals.x,
                            smd.Triangles[i].Vertices[1].Normals.y,
                            smd.Triangles[i].Vertices[1].Normals.z);
          VertexNormals.a =
              TransformRotationMatrix *
              maxon::Vector(smd.Triangles[i].Vertices[2].Normals.x,
      ...
      if (config.IncludePolySelections) {
        IF_PROFILING(Benchmark CreateSMDBuildMeshSelectionTagBench(
            "CreateSMD-BuildMesh-SelectionTag"));
        /* Selection Tags */
      ...
      if (config.IncludeUVW) {
        IF_PROFILING(Benchmark CreateSMDBuildMeshUVWTagBench(
            "CreateSMD-BuildMesh-UVWTag"));
        /* UVW Tag */
      ...
      if (config.IncludeWeights) {
        IF_PROFILING(Benchmark CreateSMDBuildMeshWeightTagBench(
            "CreateSMD-BuildMesh-WeightTag"));
        ...
        Int32 PolyIndex = 0;
        for (const auto &Poly : smd.Triangles) {
          auto GetParentWeight =
              [](const std::vector<SMDTypes::WeightmapEntry> &WeightMap)
              -> float {
            float sum = 0;
          ...
          float CParentWeight =
              GetParentWeight(Poly.Vertices[2].WeightMapEntries);
          if (CParentWeight)
            WeightTag->SetWeight(Poly.Vertices[2].ParentBone,
                                 PolygonArr[PolyIndex].a, CParentWeight);
          ...
          float BParentWeight =
              GetParentWeight(Poly.Vertices[1].WeightMapEntries);
          if (BParentWeight)
            WeightTag->SetWeight(Poly.Vertices[1].ParentBone,
                                 PolygonArr[PolyIndex].b, BParentWeight);
          ...
          float AParentWeight =
              GetParentWeight(Poly.Vertices[0].WeightMapEntries);
          if (AParentWeight)
            WeightTag->SetWeight(Poly.Vertices[0].ParentBone,
                                 PolygonArr[PolyIndex].c, AParentWeight);
...
Bool SMDLoaderData::GetDEnabling(GeListNode *node, const DescID &id,
                                 const GeData &data, DESCFLAGS_ENABLE flags,
  ...
  case SMD_LOADER_IMPORT_WEIGHTS: {
    if (!(GetParam(SMD_LOADER_IMPORT_MESH).GetBool() &&
...
Bool SMDLoaderData::GetDParameter(GeListNode *node, const DescID &id,
                                  GeData &t_data, DESCFLAGS_GET &flags) {
  if (node == nullptr)
    return false;
...
NodeData *SMDLoaderData::Create() { return NewObjClear(SMDLoaderData); }

Bool SMDLoaderData::RegisterPlugin() {
  if (!RegisterSceneLoaderPlugin(ID_SMDLOADER, "Studiomdl Data (SMD) Loader"_s,
                                 0, SMDLoaderData::Create, "Fsmdloader"_s))
    return false;
