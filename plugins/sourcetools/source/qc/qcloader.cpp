...
Bool QCLoaderData::Identify(BaseSceneLoader *node, const Filename &name,
                            UChar *probe, Int32 size) {
  // QuakeC is an ASCII format with no FourCC/Header so we will use the file
  // extension
  return (name.GetSuffix().ToLower() == "qc") ? true : false;
}

FILEERROR QCLoaderData::Load(BaseSceneLoader *node, const Filename &name,
  ...
  Bool ParamImportMaterials =
      GetParam(QC_LOADER_SOURCETYPE).GetInt32() == SOURCETYPE_NONE ? false
                                                                   : true;
  ...
  for (const auto &bgm : qc.BodyGroups) {
    ...
    if (SMDConfig.IncludeMesh) {
      if (bgm.Blank) {
        ...
        doc->InsertObject(BlankNull, BodyGroupNull,
                          BodyGroupNull->GetDownLast());
      }
    }
  }
  ...
  if (ParamImportMaterials && SMDConfig.IncludeMesh) {
    ...
    for (auto &dir : cdmat)
      if (!ResolveLocalFilepath(
              dir, std::vector<Filename>({MaterialRootDirectory}), true))
    ...
    for (auto &Mat : MaterialMap) {
      ...
      for (auto &PTag : Mat.second) {
        ...
        TTag->SetParameter(TEXTURETAG_RESTRICTION, PTag->GetName(),
                           DESCFLAGS_SET::NONE);
        TTag->SetParameter(TEXTURETAG_PROJECTION, TEXTURETAG_PROJECTION_UVW,
                           DESCFLAGS_SET::NONE);
      }
    }
  }
  ...
  if (ParamImportIK && qc.IkRules.size() > 0) {
      ...
      for (auto ik = Iks.begin(); ik != Iks.end(); ik++) {
        if (ik->EndBone == BoneName) {
          ...
          if (StartBone == nullptr) {
            String msg = "IK could not get start bone. IK Name: " + ik->Name +
                         " -- End Bone: " + ik->EndBone;
            LogError(msg);
            break;
          }
          ...
          ikTag->SetParameter(DescID(ID_CA_IK_TAG_TIP), bone.second.Object,
                              DESCFLAGS_SET::NONE);
          ikTag->SetParameter(DescID(ID_CA_IK_TAG_TARGET), ikTarget,
                              DESCFLAGS_SET::NONE);
          ikTag->SetParameter(DescID(ID_CA_IK_TAG_SOLVER),
                              ID_CA_IK_TAG_SOLVER_3D, DESCFLAGS_SET::NONE);
    ...
    for (auto &ik : Iks) {
      String msg = "Could not find IK end bone. IK Name: " + ik.Name +
                   " -- End Bone: " + ik.EndBone;
      LogError(msg);
    }
  }
...
Bool QCLoaderData::GetDEnabling(GeListNode *node, const DescID &id,
...
Bool QCLoaderData::GetDParameter(GeListNode *node, const DescID &id,
...
NodeData *QCLoaderData::Create() { return NewObjClear(QCLoaderData); }

Bool QCLoaderData::RegisterPlugin() {
  if (!RegisterSceneLoaderPlugin(ID_QCLOADER, "QuakeC (QC) Loader"_s, 0,
                                 QCLoaderData::Create, "Fqcloader"_s))
    return false;
