...
namespace st::qc
{
	namespace grammar
	{
		...
		struct action<BodyMeshName>
		{
			...
			{
				qc.BodyMeshes.emplace_back(Types::Mesh(In.string().c_str()));
			}
		};
		...
		struct action<BodyGroupName>
		{
			...
			static void apply(const Input& In, QuakeCFormat& qc)
			{
				qc.BodyGroups.emplace_back(Types::BodyGroup(In.string().c_str()));
			}
		};
		...
		struct action<ModelMeshName>
		{
			...
			static void apply(const Input& In, QuakeCFormat& qc)
			{
				qc.Models.emplace_back(Types::Model(In.string().c_str()));
			}
		};
		...
		struct action<ModelPropertyEyeballName>
		{
			...
			static void apply(const Input& In, QuakeCFormat& qc)
			{
				(qc.Models.end() - 1)->Properties.push_back(std::make_shared<ModelProperties::ModelPropertyEyeball>(In.string().c_str()));
			}
		};
		...
		struct action<IkName>
		{
			...
			static void apply(const Input& In, QuakeCFormat& qc)
			{
				qc.IkRules.emplace_back(Types::IkRule(In.string().c_str()));
			}
		};
		...
		struct action<CollisionMeshName>
		{
			...
			static void apply(const Input& In, QuakeCFormat& qc)
			{
				qc.PhysicsMesh = Types::Mesh("physics");
	...
	maxon::Bool ParseQC(const Filename& file, QuakeCFormat& qc)
	{
		...
		try
		{
			bOk = tao::pegtl::parse<grammar::qc_file, grammar::action>(infile, qc);
		}
