...
Bool VMTLoaderData::Identify(BaseSceneLoader* node, const Filename& name, UChar* probe, Int32 size)
{
	// VMT is an ASCII format, we will identify via extension
	return (name.GetSuffix().ToLower() == "vmt") ? true : false;
}

FILEERROR VMTLoaderData::Load(BaseSceneLoader* node, const Filename& name, BaseDocument* doc, SCENEFILTER filterflags, maxon::String* error, BaseThread* bt)
{
...
Material* VMTLoaderData::CreateVMT(const ValveMaterialType& vmt, const Settings& config)
{
...
NodeData* VMTLoaderData::Create()
{
	return NewObjClear(VMTLoaderData);
}

Bool VMTLoaderData::RegisterPlugin()
{
	if (!RegisterSceneLoaderPlugin(ID_VMTLOADER, "Valve Material Type (VMT) Loader"_s, 0, VMTLoaderData::Create, ""_s))
		return false;
