...
namespace st::vmt
{
	// TODO: Remove this, should be a simpler solution
	...
	namespace vmt_grammar
	{
		...
		struct basetexture :
			template_keyvalue_pair<pegtl::istring<
			'$', 'b', 'a', 's', 'e', 't', 'e', 'x', 't', 'u', 'r', 'e'>,
			basetexture_value>
		{};
		...
		struct action<shadertype>
		{
			...
			static void apply(const Input& in, ValveMaterialType& vmt)
			{
				if (istrcmp(in.string(), "character"))
					vmt.Shader = Types::ShaderType::CHARACTER;
				else if (istrcmp(in.string(), "lightmappedgeneric"))
					vmt.Shader = Types::ShaderType::LIGHTMAPPEDGENERIC;
				else
				{
					maxon::String err = "Unknwon shader type: "_s + in.string().c_str();
					LogError(err);
					vmt.Shader = Types::ShaderType::UNKNOWN;
				}
			}
		};
		...
		struct action<basetexture_value>
		{
			...
			static void apply(const Input& in, ValveMaterialType& vmt)
			{
				if (vmt.Parameters.find(Types::ParameterType::BASETEXTURE) != std::end(vmt.Parameters))
				{
					LogError("BaseTexture redefinition.");
					return;
				}

				vmt.Parameters[Types::ParameterType::BASETEXTURE] = String(in.string().c_str());
			}
		};
		...
		struct action<bumpmap_value>
		{
			...
			static void apply(const Input& in, ValveMaterialType& vmt)
			{
				if (vmt.Parameters.find(Types::ParameterType::BUMPMAP) != std::end(vmt.Parameters))
				{
					LogError("BumpMap redefinition.");
					return;
				}

				vmt.Parameters[Types::ParameterType::BUMPMAP] = String(in.string().c_str());
			}
		};
		...
		struct action<ssbump_value>
		{
			...
			static void apply(const Input& in, ValveMaterialType& vmt)
			{
				if (vmt.Parameters.find(Types::ParameterType::SSBUMP) != std::end(vmt.Parameters))
				{
					LogError("ssbump redefinition.");
					return;
				}

				vmt.Parameters[Types::ParameterType::SSBUMP] = bool(std::stoi(in.string()));
			}
		};
		...
		struct action<translucent_value>
		{
			...
			static void apply(const Input& in, ValveMaterialType& vmt)
			{
				if (vmt.Parameters.find(Types::ParameterType::TRANSLUCENT) != std::end(vmt.Parameters))
				{
					LogError("translucent redefinition.");
					return;
				}

				vmt.Parameters[Types::ParameterType::TRANSLUCENT] = bool(std::stoi(in.string()));
			}
		};
	}
