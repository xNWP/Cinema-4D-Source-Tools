...
public:
  static void Write(const String &msg) {
    if (g_instance == nullptr) {
      ...
      g_instance->_logger
          .AddLoggerType(maxon::TARGETAUDIENCE::ALL,
    ...
    g_instance->_logger
        .Write(maxon::TARGETAUDIENCE::ALL, msg,
               maxon::SourceLocation::NullValue())
        .UncheckedGetValue();
  }
};

void st::iLog(const String &msg) { LoggerImpl::Write(msg); }

void st::iLogError(const String &msg, const char *FILE, const char *FUNC,
                   UInt LINE) {
  String m = "ERROR: "_s;
  m += msg;
  m += "\nFILE: "_s;
  ...
  m += FUNC;
  m += "\nLINE: "_s;
  m += maxon::ToString(LINE, nullptr);

  st::iLog(m);
}

void st::iLogErrorWhat(const String &WHAT, const char *FILE, const char *FUNC,
                       UInt LINE) {
  String m = "ERRORWHAT: "_s;
  m += WHAT;
  m += "\nFILE: "_s;
  m += FILE;
  m += "\nFUNC: "_s;
