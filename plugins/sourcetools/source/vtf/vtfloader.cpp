...
namespace st::vtf
{
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_RGBA8888)
	{
		...
		const UInt32 offset = UInt32(file->GetLength()) -
			size * (UInt32(header.frames) - frame);
		...
		UChar* ColourData = new UChar[UInt32(header.width) * UInt32(header.height) * 3];
		if (!ColourData)
		{
			delete[] data;
			return IMAGERESULT::OUTOFMEMORY;
		...
		{
			ok = bm->SetAlphaPixel(Alpha, i % header.width, i / header.height, data[i * 4 + 3]); // Set A
		...
		{
			ok = bm->SetPixelCnt(0, y, header.width, ColourData + (y * header.width * 3), COLORBYTES_RGB, COLORMODE::RGB, PIXELCNT::NONE);
		}
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_ABGR8888)
	{
		...
		UChar* const ColourData = new UChar[UInt32(header.width) * UInt32(header.height) * 3];
		if (!ColourData)
		{
			delete[] data;
			return IMAGERESULT::OUTOFMEMORY;
		...
		for (UInt32 i = 0; i < size / 4 && ok; ++i)
		{
			ok = bm->SetAlphaPixel(Alpha, i % header.width, i / header.width, data[i * 4]);
	...
	header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_RGB888_BLUESCREEN)
	{
		...
		for (UInt32 y = 0; y < header.height && ok; ++y)
		{
			ok = bm->SetPixelCnt(0, y, header.width, data + (y * header.width * 3), COLORBYTES_RGB, COLORMODE::RGB, PIXELCNT::NONE);
		}
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_RGB565)
	{
		...
		UChar* const formattedData = new UChar[UInt32(header.width) * UInt32(header.height) * 3];
		if (!formattedData)
		{
			delete[] data;
			return IMAGERESULT::OUTOFMEMORY;
		...
		for (UInt32 y = 0; y < header.height && ok; ++y)
		{
			ok = bm->SetPixelCnt(0, y, header.width, formattedData + (y * header.width * 3), COLORBYTES_RGB, COLORMODE::RGB, PIXELCNT::NONE);
		}
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_I8)
	{
		...
		for (UInt32 y = 0; y < header.height && ok; ++y)
		{
			ok = bm->SetPixelCnt(0, y, header.width, data + (y * header.width), COLORBYTES_GRAY, COLORMODE::GRAY, PIXELCNT::NONE);
		}
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_IA88)
	{
		...
		for (UInt32 i = 0; i < size / 2 && ok; ++i)
			ok = bm->SetAlphaPixel(alpha, i % header.width, i / header.width, AData[i]);
		...
		// Set Color Data
		for (UInt32 y = 0; y < header.height && ok; ++y)
			ok = bm->SetPixelCnt(0, y, header.width, IData + (y * header.width), COLORBYTES_GRAY, COLORMODE::GRAY, PIXELCNT::NONE);
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_A8)
	{
		...
		for (UInt32 i = 0; i < size && ok; ++i)
			ok = bm->SetAlphaPixel(alpha, i % header.width, i / header.width, data[i]);
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_ARGB8888)
	{
		...
		for (UInt32 i = 0; i < size / 4 && ok; ++i)
		{
			...
			ok = bm->SetAlphaPixel(Alpha, i % header.width, i / header.height, data[i * 4]); // Set A
		}
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_BGRA8888)
	{
		...
		for (UInt32 i = 0; i < size / 4 && ok; ++i)
		{
			...
			ok = bm->SetAlphaPixel(Alpha, i % header.width, i / header.width, data[i * 4 + 3]); // Set A
		}
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_DXT1)
	{
		...
		// determine offset of high-res data
		const UInt32 CompressedSize = UInt32(UInt32(header.width) * UInt32(header.height) * 0.5f);
		const UInt32 offset = UInt32(file->GetLength()) -
		...
		file->Seek(offset, FILESEEK::START);
		if (file->ReadBytes(CompressedData, CompressedSize, true) != CompressedSize)
		{
		...
		// Decompress The Data
		Bool ok = true;
		for (UInt32 b = 0; b < (UInt32(header.width) * UInt32(header.height) / 16) && ok; ++b)
		{
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_DXT3)
	{
		...
		{
			// Read in Alpha Data
			std::vector<UChar> AlphaArray = ExtractAlphaEntriesDXT3(*(UInt64*)(CompressedData + (b * 16)));

			PIX565* const CompressedColour_0 = (PIX565*)(CompressedData + (b * 16) + 8);
			PIX565* const CompressedColour_1 = CompressedColour_0 + 1;
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_DXT5)
	{
		...
		{
			// Read in Alpha Data
			std::vector<UChar> AlphaArray = ExtractAlphaEntriesDXT5(*(UInt64*)(CompressedData + (b * 16)));
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_BGRX8888)
	{
		...
		const UInt32 offset = UInt32(file->GetLength()) -
			DataSize * (UInt32(header.frames) - frame);
		...
		Bool ok = true;
		for (UInt32 i = 0; (i < UInt32(header.width) * UInt32(header.height)) && ok; ++i)
		{
			ColourData[i * 3] = Data[i * 4 + 2]; // Set R
			ColourData[i * 3 + 1] = Data[i * 4 + 1]; // Set G
			ColourData[i * 3 + 2] = Data[i * 4]; // Set B
			ok = bm->SetAlphaPixel(XChannel, i % header.width, i / header.width, Data[i * 4 + 3]);
		}
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_BGRX5551)
	{
		...
		for (UInt32 i = 0; (i < UInt32(header.width) * UInt32(header.height)) && ok; ++i)
		{
			...
			ok = bm->SetAlphaPixel(XChannel, i % header.width, i / header.width, Pix->z == 1 ? 0xFF : 0x0);
		}
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_BGRA4444)
	{
		...
		// determine offset of high-res data
		const UInt32 CompressedSize = UInt32(header.width) * UInt32(header.height) * 2;
		const UInt32 offset = UInt32(file->GetLength()) -
			CompressedSize * (UInt32(header.frames) - frame);
		...
		UChar* const ColourData = new UChar[UInt32(header.height) * UInt32(header.width) * 3];
		if (!ColourData)
		{
			delete[] data;
			return IMAGERESULT::MISC_ERROR;
		...
		for (UInt32 i = 0; i < UInt32(header.width) * UInt32(header.height) && ok; ++i)
		{
			...
			ok = bm->SetAlphaPixel(Alpha, i % header.width, i / header.width, LUT4[Pix->z]); // Set A
		}
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_BGRA5551)
	{
		...
		for (UInt32 i = 0; (i < UInt32(header.width) * UInt32(header.height)) && ok; ++i)
		{
			...
			ColourData[i * 3 + 1] = LUT5[Pix->x]; // Set G
			ColourData[i * 3 + 2] = LUT5[Pix->w]; // Set B
			ok = bm->SetAlphaPixel(AlphaChannel, i % header.width, i / header.width, Pix->z == 1 ? 0xFF : 0x0);
		}
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_UV88)
	{
		...
		UChar* const ImageData = new UChar[UInt32(header.height) * UInt32(header.width) * 3];
		if (!ImageData)
		{
			delete[] data;
			return IMAGERESULT::MISC_ERROR;
		...
		for (UInt32 y = 0; y < header.height && ok; ++y)
		{
			ok = bm->SetPixelCnt(0, y, header.width, ImageData + (y * header.width * 3), COLORBYTES_RGB, COLORMODE::RGB, PIXELCNT::NONE);
		}
	...
	else if (header.highResImageFormat == Types::IMAGEFORMAT::IMAGE_FORMAT_UVWQ8888)
	{
		...
		for (UInt32 i = 0; (i < UInt32(header.width) * UInt32(header.height)) && ok; ++i)
		{
			...
			ok = bm->SetAlphaPixel(QChannel, i % header.width, i / header.width, Data[i * 4 + 3]);
		}
